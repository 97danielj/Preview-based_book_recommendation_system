...
path = 'C:/Users/DanielJeong/PycharmProjects/Preview-based_book_recommendation_system/97danielj_progress'
os.chdir(path)
def load_analyzed_data():
    ...
    #파일 경로
    analyzed_data_path = path+"/analyzed_data.json"

    with open(analyzed_data_path, encoding='utf8') as f:
...
def load_preprocessed_data()-> pd.DataFrame:
    ...
    # 파일 경로
    #pre_processed_data_path = "df_to_json.json"
    # DataFrame 객체로 변환 후 반환
    preprocessed_data_path = path+"/df_to_json.json"
    #with open(preprocessed_data_path, encoding='utf8') as f:
    #    df = pd.DataFrame([json.loads(l) for l in f.readlines()])
    return pd.read_json(preprocessed_data_path)

def load_doc2vec_data():
    """doc2vec 모델을 가져온다"""
    return doc2vec.Doc2Vec.load(path+"/dart.doc2vec")
